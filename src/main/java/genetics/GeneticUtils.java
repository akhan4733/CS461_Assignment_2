...
public class GeneticUtils {
    ...
    public static double calculateFitnessOfGene(GeneticInformation targetGeneticInformation, Individual individual) {
        double fitnessScore = 0;
        List<GeneticInformation> remainingGeneticInformations = getRemainingGenes(targetGeneticInformation, individual);

        //Activity is scheduled at the same time in the same room as another of the activities: -0.5
        if(remainingGeneticInformations.stream().anyMatch(geneticInformation -> isActivitySamePlaceAndTime(geneticInformation, targetGeneticInformation))) {
            fitnessScore -= 0.5;
        }
        ...
        // Activity facilitator is scheduled for only 1 activity in this time slot: + 0.2
        if(remainingGeneticInformations.stream().noneMatch(geneticInformation -> doActivitiesHaveSameTimeAndFacilitator(geneticInformation, targetGeneticInformation))) {
            fitnessScore += 0.2;
        }
        // Activity facilitator is scheduled for more than one activity at the same time: - 0.2
        else  {
        ...
        long scheduledActivityCountForFacilitator = remainingGeneticInformations.stream()
    ...
    public static List<GeneticInformation> getRemainingGenes(GeneticInformation targetGeneticInformation, Individual individual) {
    ...
    public static List<GeneticInformation> getConsecutiveTimes(List<GeneticInformation> geneticInformationList) {
    ...
    /*
         ...
         */
    public static boolean isRoomAvoidanceMet(GeneticInformation geneticInformation1, GeneticInformation geneticInformation2, Room room1, Room room2) {
    ...
    public static boolean isActivitySamePlaceAndTime(GeneticInformation geneticInformation, GeneticInformation targetGeneticInformation) {
    ...
    public static boolean doActivitiesHaveSameTimeAndFacilitator(GeneticInformation geneticInformation, GeneticInformation targetGeneticInformation) {
